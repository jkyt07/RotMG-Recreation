...
var roomStart = instance_create_layer(8*24*irandom_range(1,floor(room_width/(8*24)) - 1),8*24*irandom_range(1,floor(room_width/(8*24)) - 1), "InstancesB", objCowardice)
//roomStart.x = 8*(11.5 + 24)
//roomStart.y = 8*(11.5 + 24)
var dends = irandom_range(0,1) + irandom_range(0,1) + irandom_range(0,1)

var dungeonLength = irandom_range(5,12)
var curLength = 0
var curDir = -1
...
while(curLength < dungeonLength)
{
	...
	{
		var nextRoom = irandom_range(0,1)
		...
		else 
		{
			...
			if(nextRoom = 1 and nextDir == curDir)
			{
				pcaveHallway(nextX, nextY, openings[0])
			}
			else
			{
